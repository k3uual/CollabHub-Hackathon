...
cards.forEach(card => {
    let countdown = card.querySelector('.left');
    let timeLeft = parseInt(countdown.dataset.timeleft);

    let timerInterval = setInterval(() => {
        if (timeLeft <= 0) {
            clearInterval(timerInterval);
            card.style.display = 'none'; // Remove card
        } else {
            timeLeft--;
            countdown.innerHTML = `<i class="bi-clock-fill"></i> ${timeLeft} second(s) left`;
        }
    }, 1000);
});
    </script>
